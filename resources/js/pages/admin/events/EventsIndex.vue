...
const deleteEvent = () =>{
  ...
  axios.delete(`/events/${userIdBeingDeleted}`)
  .then(()=>{
...
<div v-if="!loadingDiv">
  ...
  <tbody v-if="events.data.length > 0">
    <tr v-for="(event,index) in events.data" :key="event.id">
      ...
      <td>
        <router-link :to="'/admin/events/'+event.id+'/edit'"><i class="ti ti-edit"></i></router-link>
        <router-link :to="'/admin/events/'+event.id"><i class="ti ti-eye"></i></router-link>
