...
const form = reactive({
 ...
 const getAssociateList = async (page=1) => {
  axios.get(`/manager-associates?page=${page}`,{
    params:{
...
const deleteAssociate = () =>{
  ...
  axios.delete(`/manager-associates/${userIdBeingDeleted}`)
  .then(()=>{
...
<div v-if="!loadingDiv">
    <div class="card">
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between">
              <router-link to="/manager/associates/create"  class="btn btn-primary m-2">
  ...
  <tbody v-if="associates.data.length > 0">
    <tr v-for="(associate,index) in associates.data" :key="associate.id">
      ...
      <td>
        <router-link :to="'/manager/associates/'+associate.id+'/edit'"><i class="ti ti-edit"></i></router-link>
        <router-link :to="'/manager/associates/'+associate.id"><i class="ti ti-eye"></i></router-link>
