...
const form = reactive({
 ...
 const getEventList = async (page=1) => {
  axios.get(`/associate-events?page=${page}`,
  {
    params: {
...
const createCategoryFunction = (values, {resetForm}) => {
  axios.post('/associate-events',values).then((response)=>{
...
const deleteEvent = () =>{
  ...
  axios.delete(`/associate-events/${userIdBeingDeleted}`)
  .then(()=>{
...
<div v-if="!loadingDiv">
    <div class="card">
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between">
              <!-- <router-link to="/admin/events/create"  class="btn btn-primary m-2">
  ...
  <tbody v-if="events.data.length > 0">
    <tr v-for="(event,index) in events.data" :key="event.id">
      ...
      <td>
        <!-- <router-link :to="'/associate/events/'+event.id+'/edit'"><i class="ti ti-edit"></i></router-link> -->
        <router-link :to="'/associate/events/'+event.id"><i class="ti ti-eye"></i></router-link>
