...
const form = reactive({
 ...
 const getQuoteList = async (page = 1) => {
  axios.get(`/manager-quotes?page=${page}`,
  {
        params:{
...
const deleteQuote = () =>{
  ...
  axios.delete(`/manager-quotes/${userIdBeingDeleted}`)
  .then(()=>{
...
<div v-if="!loadingDiv">
    <div class="card">
          <div class="card-body">
            ...
            <router-link to="/manager/quotes/create"  class="btn btn-primary m-2">
  ...
  <tbody v-if="quotes.data.length > 0">
    <tr v-for="(quote,index) in quotes.data" :key="quote.id">
      ...
      <td>
        <router-link :to="'/manager/quotes/'+quote.id+'/edit'"><i class="ti ti-edit"></i></router-link>
        <router-link :to="'/manager/quotes/'+quote.id"><i class="ti ti-eye"></i></router-link>
