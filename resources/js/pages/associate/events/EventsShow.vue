...
const getEvent = () => {
  axios.get(`/associate-events/+${router.currentRoute.value.params.id}`)
       .then((response)=>{
        loading.value=false;
...
const createCategoryFunction = (values, {resetForm}) => {
  axios.post('/associate-categories',values).then((response)=>{

    // categories.value.unshift(response.data);
    // $('#createCategory').modal('hide');
    resetForm();
    router.push({ path: '/associate/categories' })
  })
};
...
<template>
    <div v-if="!loading">
        <router-link to="/associate/events" >
