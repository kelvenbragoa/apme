...
const form = reactive({
 ...
 const getQuoteList = async (page=1) => {
  axios.get(`/associate-quotes?page=${page}`,
    {
        params:{
...
const createCategoryFunction = (values, {resetForm}) => {
  axios.post('/associate-quotes',values).then((response)=>{
...
const deleteQuote = () =>{
  ...
  axios.delete(`/associate-quotes/${userIdBeingDeleted}`)
  .then(()=>{
...
<div v-if="!loadingDiv">
    <div class="card">
          <div class="card-body">
            ...
            <!-- <router-link to="/associate/quotes/create"  class="btn btn-primary m-2">
  ...
  <tbody>
    <tr v-for="(quote,index) in quotes.data" :key="quote.id">
      ...
      <td>
        <!-- <router-link :to="'/associate/quotes/'+quote.id+'/edit'"><i class="ti ti-edit"></i></router-link> -->
        <router-link :to="'/associate/quotes/'+quote.id"><i class="ti ti-eye"></i></router-link>
